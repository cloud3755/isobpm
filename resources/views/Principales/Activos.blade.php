...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header text-center" style="font-weight: bold; text-shadow: 1px 1px #222; color:#0070B0;font-family: 'LeagueGothic';word-spacing: 5px; letter-spacing: 2px; border-bottom: none">Activos de Informacion</h1>
    </div>
</div>
              ...
              <br>
                <table width="100%" class="table table-responsive table-striped table-bordered table-hover" id="datos">
                  <thead style='background-color: #868889; color:#FFF'>
                    <tr>
                      <th>  <div class="th-inner sortable both">    Id_unico  </div></th>
                  ...
                  <tbody id = "myTable">
                    ...
                    <tr>
                      ...
                      <td>  <?=$Activo->usernombre?></td>
                      <td>
                        <button type="button" class="btn btn-primary" value = "<?=$Activo->id?>" data-toggle="modal" data-target="#modaledit" onclick="Editar(this);"><i class="glyphicon glyphicon-edit"></i></button>
                      </td>
                      <td>
                        <form class="" action="/activos/destroy/{{ $Activo->id }}" method="post">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
                          <button type="submit" class="btn btn-danger" id="btnpro" style="font-family: Arial;" onclick="
                            return confirm('Estas seguro de eliminar el documento <?=$Activo->nombre?>?')"><i class="fa fa-trash"></i></button>
                        </form>
                      </td>
                    </tr>
                    <?php endforeach ?>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
              <form class="" action="/activos/store" method="post" accept-charset="UTF-8" enctype="multipart/form-data">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <div class="row">
                  <div class="col-md-12 col-sm-12">
                      <h2><label for="Usuario" class="control-label">Nombre:</label></h2>
                          <input class="form-control input-lg" id="nombre" type="Text" placeholder="Nombre" name="nombre" required>
                      </div>
                    <div class="col-md-12 col-sm-12">
                    <h2><label for="tipo" class="control-label" >Descripcion:</label></h2>
                      <textarea class="form-control input-lg" id="descripcion" type="Text" placeholder="Agrega una descripcion del archivo" name="descripcion" required> </textarea>
                    </div>
                    <div class="col-md-12 col-sm-12">
                      <h2><label for="tipo" class="control-label" >Responsable:</label></h2>
                      <select class="form-control input-lg" name="nresponsable" id="nresponsable">
...
<div class="modal fade" id="modaledit" tabindex="-1" role="dialog" style="background-color:gray">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-hidden="true">X</button>
                <h3 class="modal-title">ACTUALIZAR ACTIVO</h3>
            </div>
            <div class="modal-body">
              ...
              <div class="row">
                ...
                <div class="col-md-12 col-sm-12">
                  <h2><label for="tipo" class="control-label" >(*) Descripcion:</label></h2>
                    <textarea class="form-control input-lg" id="edescripcion" type="Text" placeholder="Agrega una descripcion del archivo" name="edescripcion" required></textarea>
                </div>
                <div class="col-md-12 col-sm-12">
                  <h2><label for="tipo" class="control-label" >Responsable:</label></h2>
                  <select class="form-control input-lg" name="eresponsable" id="eresponsable">
                      <?php foreach ($User as $Users): ?>
                          <option value="<?=$Users['id']?>"><?=$Users['nombre']?></option>
                      <?php endforeach ?>
                  </select>
...
function Editar(btn){
  var route = "/activos/"+btn.value+"/edit";

  $.get(route, function(res){
    $("#enombre").val(res.nombre);
    $("#id").val(res.id);
    $("#edescripcion").val(res.descripcion);
    $('#eresponsable option[value="' + res.responsable + '"]').attr("selected", "selected");
  });
